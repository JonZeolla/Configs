...
brew update
brew install go git wget gnupg2 npm yarn nmap swig cmake openssl jq azure-cli hashcat shellcheck packer dos2unix testssl ttygif tree vim imagemagick ruby autoconf automake libtool gnu-tar pandoc aircrack-ng bash libextractor fortune cowsay lolcat awscli terraform kubectl nuget screen zsh bison zmap watch jupyter asciinema coreutils graphviz wakeonlan grep hadolint coreutils yara neovim neo4j kubectx git-lfs aquasecurity/trivy/trivy ncrack fzf dive ykman minikube octant krew sha3sum tor tor-browser libxml2 libxmlsec1 pkg-config age syft grype beekeeper-studio pyenv ansible sigstore/tap/cosign crane act just colordiff rust logitech-options kind cantino/mcfly/mcfly direnv helm tektoncd-cli cue-lang/tap/cue sigstore/tap/gitsign cfn-lint ansible-lint gofmt rustfmt scc gh quarto sigstore/tap/gitsign-credential-cache screenflow yq gitter docker-slim
npm install -g electron-packager
brew install --cask vagrant google-chrome sublime-text wireshark iterm2 slack steam firefox the-unarchiver gpg-suite owasp-zap keycastr balenaEtcher drawio visual-studio-code little-snitch micro-snitch launchbar hazel bloodhound xquartz surge keka microsoft-office evernote wire chef/chef/inspec postman paragon-extfs pdftotext obs signal toggle-track gimp lens meld quik microsoft-teams lastpass yt-music discord google-drive intune-company-portal logitech-presentation parallels rancher docker chromedriver
...
pip3 install bcrypt impacket pylint termcolor flake8 defusedxml validators mypy black pytest-cov coverage virtualenv yamllint bandit scandir lxml grip cookiecutter pipx c7n pre-commit gitpython pyyaml flynt
