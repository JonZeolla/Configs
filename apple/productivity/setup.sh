...
brew install --cask vagrant google-chrome sublime-text wireshark iterm2 slack steam firefox the-unarchiver gpg-suite owasp-zap keycastr balenaEtcher drawio visual-studio-code little-snitch micro-snitch launchbar hazel bloodhound xquartz surge keka microsoft-office evernote wire chef/chef/inspec postman paragon-extfs pdftotext obs signal toggle-track gimp lens meld quik microsoft-teams lastpass discord google-drive intune-company-portal logitech-presentation parallels rancher docker chromedriver spotify
