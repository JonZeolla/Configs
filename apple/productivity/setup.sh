...
brew install --cask vagrant virtualbox google-chrome sublime-text wireshark iterm2 slack steam firefox the-unarchiver gpg-suite docker owasp-zap keycastr balenaEtcher drawio visual-studio-code little-snitch micro-snitch launchbar hazel bloodhound xquartz surge keka microsoft-office evernote wire chef/chef/inspec postman paragon-extfs pdftotext obs signal toggle-track gimp lens meld quik microsoft-teams lastpass yt-music discord
