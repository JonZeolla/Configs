...
brew update
brew install python3 go maven@3.3 maven git wget gnupg2 ant npm yarn nmap swig cmake openssl jq azure-cli hashcat shellcheck packer nvm dos2unix testssl ttygif tree vim imagemagick ruby autoconf automake libtool gnu-tar pandoc aircrack-ng bash libextractor fortune cowsay lolcat awscli terraform kubectl nuget php screen zsh heroku/brew/heroku bison zmap watch jupyter asciinema coreutils libnfc mfoc powerlevel9k logstash pipenv graphviz wakeonlan grep hadolint coreutils yara neovim neo4j kubectx git-lfs aquasecurity/trivy/trivy ncrack fzf dive yubico-yubikey-manager yubico-authenticator fujitsu-scansnap-manager-ix500 minikube google-chrome-canary nasm octant krew sha3sum tor tor-browser
npm install -g electron-packager
brew install --cask vagrant virtualbox google-chrome sublime-text wireshark mysqlworkbench iterm2 slack steam firefox the-unarchiver gpg-suite docker burp-suite balenaEtcher atom veracrypt beyond-compare drawio visual-studio-code little-snitch micro-snitch launchbar Keyboard-Maestro hazel bloodhound xquartz playonmac tunnelblick google-cloud-sdk surge keka microsoft-office evernote wire chef/chef/inspec thunderbird intellij-idea metasploit quicklook-json postman paragon-extfs pdftotext obs signal toggle-track gimp lens meld
